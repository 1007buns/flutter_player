...
class Player extends FijkPlayer {
  ...
  static String _cachePath =
      '/storage/emulated/0/Android/data/com.example.mc/files';
  ...
  Future<void> setDataSource(String path,
    ...
    if (File(videoPath).existsSync()) {
      // 如果二级缓存存在，则直接用本地缓存的视频文件播放
      path = videoPath;
      print('manto-> play cache video :$path');
    } else if (enableCache) {
      // 走三级缓存，并添加到二级缓存到本地磁盘中
      // 设置播放器缓存的步骤
      //1.增加视频path的前缀
      path = '$cache_switch$path';
      // 2.通过setOption设置cache_file_path
      setOption(FijkOption.formatCategory, 'cache_file_path', videoPath);
      print('manto-> play http with cache:$path');
    } else {
      print('manto-> play http :$path');
    }
  ...
  String getVideoCatchPath(String url, String cachePath) {
    ...
    print('manto-> get video path: $path');
    return path;
  }
}
